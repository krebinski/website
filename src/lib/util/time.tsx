...
// TODO: Use ITNL instead?
export const formatRelativeDate = (date:Date): string => {
    const now = Date.now();
    const secondsPast = (now - date.getTime()) / 1000;

    if (secondsPast < 60) 
        return formatRelativeUnit(secondsPast, t("time.second"));
    if (secondsPast < 3600)
        return formatRelativeUnit(secondsPast /60, t("time.minute"));
    if (secondsPast < 86400)
        return formatRelativeUnit(secondsPast / 3600, t("time.hour"));

    const days = secondsPast / 86400;
    if (days > 7) return formatISODate(date);

    return formatRelativeUnit(days, t("time.day"));
};
