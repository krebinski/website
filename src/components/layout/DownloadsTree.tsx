      ...
      {project?.versions
        ...
        ?.map((version) => (
          <button
            key={version}
            className={clsx(
              "pl-6 py-1 rounded-md hover:bg-purple-100 hover:dark:bg-gray-900 transition-colors text-gray-800 dark:text-gray-200 block w-full text-left",
              selectedProject === id &&
...
const DownloadsTree = (props: DownloadsTreeProps) => {
  return (
    <nav className="w-50 p-2 border-r border-gray-300 overflow-auto">
      <ProjectSubTree id="mafaka" name="Mafaka" {...props} />
    </nav>
  );
};
